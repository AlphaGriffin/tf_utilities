...
class Build_Model(object):
    ...
    @staticmethod
    def BUILD_FC_LAYER(x_image, num_fc, num_final):
        """
    ...
    def build_inputs():
        with tf.variable_scope('inputs'):
            ...
            tf.add_to_collection(['global_step', 'inputs'], global_step)
            tf.add_to_collection(['learn_rate', 'inputs'], learn_rate)
            tf.add_to_collection(['input_tensor', 'inputs'], Input_Tensor_Image)
            tf.add_to_collection(['label_tensor', 'inputs'], Input_Tensor_Labels)
            tf.add_to_collection(['keep_prob', 'inputs'], keep_prob)
        return Input_Tensor_Image, input_tensor, learn_rate

    def build_outputs(x_image, num_conv, num_fc, num_outputs):
        with tf.variable_scope("conv_layers"):
            x_image, num_features = BUILD_CONV_LAYERS(x_image, num_conv)
        with tf.variable_scope("Flat_layer"):
            x_image, features = flatten_layer(x_image)
        with tf.variable_scope("FC_layers"):
            final_layer = BUILD_FC_LAYER(x_image, num_fc, num_final=num_outputs)
        return final_layer

    def training_method(x_image, input_tensor, learn_rate):
