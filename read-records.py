...
# ==============================================================================
'''
python ./read-records.py /tmp/train.record
'''
...
def decode(batch_serialized_examples):
  ...
  image_data = features['image_data']
  label_data = features['label_data']

  # index2 = tf.sparse_tensor_to_dense(index)

  return height, width, channels, actions, image_data, label_data
...
def read_records():
  ...
  with tf.Graph().as_default():
    ...
    tf_record_pattern = ["{}".format(os.path.join(os.getcwd(), 'dataset', 'mupen64plus.tfrecords'))]
    data_files = tf.gfile.Glob(tf_record_pattern)
    height, width, channels, actions, image_data, label_data = batch_inputs(
      data_files, 
    ...
    try:
      ...
      while not coord.should_stop():
        height_, width_, channels_, actions_, image_data_, label_data_ = sess.run([height,
                                                                                   width,
                                                                                   channels,
                                                                                   actions,
                                                                                   image_data,
